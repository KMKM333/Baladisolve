...
`manifest.webmanifest`, `sw.js` and the icons in `images/` make the site
installable on a phone. The service worker is network-first for the page
itself (a deploy is never masked by a stale copy) and caches third-party
assets as they are seen. `netlify.toml` serves `sw.js` with no-cache.

## Database (Supabase), step 1

`supabase/schema.sql`, `policies.sql` and `seed.sql` (generated by
`scripts/export-seed.mjs`, never hand-edited) are the tables the arrays in
index.html will move into. The site has a config block near the end,
`window.MANARA_SUPABASE`; empty means built-in data, filled means requests,
custody trail, pledges and chats load from the database with a fallback.
Stage transitions are not in the database yet — that is step 2. Steps for the
account owner are in `supabase/README.md`.
